...
describe('buildRenderList lifted nodes', () => {
  /** A scrolling row whose middle card is lifted out of it. */
  function liftedRow(lift: boolean) {
    const h = new RenderHarness();
    const scroll = h.createNode('scroll', UiNodeType.ScrollView);
    scroll.setProperty('width', 200);
    scroll.setProperty('height', 100);
    ...
    const picture = box(h, 'picture', { width: 60, height: 100, backgroundColor: '#bbb', flexShrink: 0 });
    const c = box(h, 'c', { width: 60, height: 100, backgroundColor: '#ccc', flexShrink: 0 });
    if (lift) {
      picture.setProperty('lift', true);
    }
    h.graph.appendChild(scroll, a);
    h.graph.appendChild(scroll, picture);
    h.graph.appendChild(scroll, c);
    h.layout(scroll, Constraints.loose(800, 600));
    return buildRenderList(scroll, h.engine, h.measurer, 800, 600, 1);
  }

  it('emits a lifted node last and with no scissor', () => {
    const list = liftedRow(true);
    const commands = primitiveCommands(list);
    const last = commands[commands.length - 1]!;
    expect(last.scissor).toBeNull();
    // The row's three cards, the lifted one moved to the end.
    expect([0, 1, 2].map(index => readInstance(list, index).x)).toEqual([0, 120, 60]);
  });

  it('leaves an unlifted node in tree order, inside the row scissor', () => {
    const list = liftedRow(false);
    const commands = primitiveCommands(list);
    for (const command of commands) {
      expect(command.scissor).toEqual({ x: 0, y: 0, width: 200, height: 100 });
    }
    expect([0, 1, 2].map(index => readInstance(list, index).x)).toEqual([0, 60, 120]);
  });

  it('drops the rounded clip chain a lifted node was inside', () => {
    const h = new RenderHarness();
    const scroll = h.createNode('scroll', UiNodeType.ScrollView);
    scroll.setProperty('width', 200);
    scroll.setProperty('height', 100);
    scroll.setProperty('borderRadius', 12);
    const picture = box(h, 'picture', { width: 60, height: 100, backgroundColor: '#bbb', flexShrink: 0, lift: true });
    h.graph.appendChild(scroll, picture);
    h.layout(scroll, Constraints.loose(800, 600));
    const list = buildRenderList(scroll, h.engine, h.measurer, 800, 600, 1);
    // The container paints nothing of its own, so the lifted child is
    // the only instance, and it names no clip: the rounded chain the
    // shader would have walked is exactly what `lift` escapes.
    expect(readInstance(list, 0).clipIndex).toBe(-1);
  });
});
