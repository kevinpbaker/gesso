...
describe('UiGraphBuilder reconciliation', () => {
  ...
  // Unkeyed children are matched by walking the parent's existing
  // children for the first unclaimed one of the right type, which the
  // matcher does from an advancing cursor rather than from the start.
  // These pin the orderings that cursor has to keep answering the same
  // way: a list whose contents move, one that grows in the middle, one
  // whose types change part-way along, and one where a keyed sibling
  // claims a node out of order and leaves a hole behind it.
  describe('matching unkeyed children', () => {
    it('keeps unkeyed children in place when the list is reversed', () => {
      const { builder } = createBuilder();
      const root = builder.build(Column(Text({ text: 'A' }), Text({ text: 'B' }), Text({ text: 'C' })));
      const original = getChildren(root);
      const updated = builder.build(Column(Text({ text: 'C' }), Text({ text: 'B' }), Text({ text: 'A' })));
      const children = getChildren(updated);
      // Nothing identifies an unkeyed child but its position, so a
      // reversal is three property writes rather than three moves.
      expect(children[0]).toBe(original[0]);
      expect(children[1]).toBe(original[1]);
      expect(children[2]).toBe(original[2]);
      expect(children.map(child => child.getProperty('text'))).toEqual(['C', 'B', 'A']);
    });

    it('reuses the leading unkeyed children when one is inserted in the middle', () => {
      const { builder } = createBuilder();
      const root = builder.build(Column(Text({ text: 'A' }), Text({ text: 'C' })));
      const original = getChildren(root);
      const updated = builder.build(Column(Text({ text: 'A' }), Text({ text: 'B' }), Text({ text: 'C' })));
      const children = getChildren(updated);
      expect(children).toHaveLength(3);
      expect(children[0]).toBe(original[0]);
      expect(children[1]).toBe(original[1]);
      expect(children[2]).not.toBe(original[0]);
      expect(children[2]).not.toBe(original[1]);
      expect(children.map(child => child.getProperty('text'))).toEqual(['A', 'B', 'C']);
    });

    it('replaces only the child whose type changed mid-list', () => {
      const { graph, builder } = createBuilder();
      const root = builder.build(Column(Text({ text: 'A' }), Text({ text: 'B' }), Text({ text: 'C' })));
      const original = getChildren(root);
      const updated = builder.build(Column(Text({ text: 'A' }), Button({ text: 'B' }), Text({ text: 'C' })));
      const children = getChildren(updated);
      expect(children).toHaveLength(3);
      expect(children[0]).toBe(original[0]);
      expect(children[1].type).toBe(UiNodeType.Button);
      expect(children[1]).not.toBe(original[1]);
      expect(graph.hasNode(original[1].id)).toBe(true);
      // The Text that followed the replaced child is still a candidate
      // for the trailing definition, so it survives rather than being
      // rebuilt behind the type change.
      expect(children[2]).toBe(original[2]);
      expect(children.map(child => child.getProperty('text'))).toEqual(['A', 'B', 'C']);
    });

    it('matches unkeyed children around a keyed sibling that moved to the front', () => {
      const { builder } = createBuilder();
      const root = builder.build(Column(Text({ text: 'P' }), Text({ text: 'K', key: 'k' }), Text({ text: 'Q' })));
      const original = getChildren(root);
      const updated = builder.build(Column(Text({ text: 'K', key: 'k' }), Text({ text: 'P' }), Text({ text: 'Q' })));
      const children = getChildren(updated);
      // The key claims the middle node first, leaving a hole the
      // unkeyed definitions have to step over to reach the third.
      expect(children[0]).toBe(original[1]);
      expect(children[1]).toBe(original[0]);
      expect(children[2]).toBe(original[2]);
      expect(children.map(child => child.getProperty('text'))).toEqual(['K', 'P', 'Q']);
    });

    it('picks every child of the wanted type out of an alternating list', () => {
      const { graph, builder } = createBuilder();
      const root = builder.build(
        Column(
          Button({ text: 'b0' }),
      ...
      const updated = builder.build(Column(Text({ text: 'A' }), Text({ text: 'B' }), Text({ text: 'C' })));
      const children = getChildren(updated);
      // Each definition takes the next Text along, stepping over the
      // Buttons between them; the Buttons match nothing and go. The
      // leading Button is the interesting one: it is never claimed, so
      // it can never be skipped on the grounds of being claimed.
      expect(children).toHaveLength(3);
      expect(children[0]).toBe(original[1]);
      expect(children[1]).toBe(original[3]);
      expect(children[2]).toBe(original[5]);
      expect(children.map(child => child.getProperty('text'))).toEqual(['A', 'B', 'C']);
      expect(graph.hasNode(original[0].id)).toBe(false);
      expect(graph.hasNode(original[2].id)).toBe(false);
      expect(graph.hasNode(original[4].id)).toBe(false);
    });

    it('matches a keyed sibling and an unkeyed one that swapped places', () => {
      const { builder } = createBuilder();
      const root = builder.build(
        Column(Text({ text: 'A', key: 'a' }), Text({ text: 'X' }), Text({ text: 'B', key: 'b' }))
      );
      const original = getChildren(root);
      const updated = builder.build(
        Column(Text({ text: 'A', key: 'a' }), Text({ text: 'B', key: 'b' }), Text({ text: 'X' }))
      );
      const children = getChildren(updated);
      expect(children[0]).toBe(original[0]);
      expect(children[1]).toBe(original[2]);
      expect(children[2]).toBe(original[1]);
      expect(children.map(child => child.getProperty('text'))).toEqual(['A', 'B', 'X']);
    });
  });
