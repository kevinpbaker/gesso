...
| What a screen reader is given is what the tree says | Chrome's computed accessibility tree per route, written to a committed report with every control named and the Tab order walked | `pnpm check:a11y`; VoiceOver and NVDA have not been run, and `decisions/0034` says so                     |
