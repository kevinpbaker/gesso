...
above follows it — and it is following the page's toggle, not your
operating system's setting, which is the distinction the API exists for.
...
person's choice is application state, and where it is stored — a file, a
database, `localStorage` behind a shell request — is the application's
decision.
