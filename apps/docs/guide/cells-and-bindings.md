...
A parent can pass a plain value or an Observable — every prop is
...
- `text`, `image` — content: repaint.
- `backgroundColor`, `opacity`, `borderColor` — paint: repaint, no
  layout.
- `width`, `gap`, `flexGrow`, `padding` — layout: a layout pass from the
  nearest ancestor that can absorb the change.
...
from one emission, with no list to re-render and no diff — which is why
