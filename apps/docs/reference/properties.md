...
There are 118 properties. Every element accepts every one of them, because a property is a slot on a node rather than something a particular element declares. A property nothing sets costs nothing: it is not stored, and it resolves to the default below.
...
| Property | Default | Inherited | Invalidates | What it does |
| -------- | ------- | --------- | ----------- | ------------ |
| `padding` | `unset` | no | Layout | Space inside the border on all four sides. |
| `paddingTop` | `unset` | no | Layout | Overrides `padding` on the top edge. |
| `paddingRight` | `unset` | no | Layout | Overrides `padding` on the right edge. |
| `paddingBottom` | `unset` | no | Layout | Overrides `padding` on the bottom edge. |
| `paddingLeft` | `unset` | no | Layout | Overrides `padding` on the left edge. |
| `margin` | `unset` | no | Layout | Space outside the box on all four sides. Margins do not collapse. |
| `marginTop` | `unset` | no | Layout | Overrides `margin` on the top edge. |
| `marginRight` | `unset` | no | Layout | Overrides `margin` on the right edge. |
| `marginBottom` | `unset` | no | Layout | Overrides `margin` on the bottom edge. |
| `marginLeft` | `unset` | no | Layout | Overrides `margin` on the left edge. |
