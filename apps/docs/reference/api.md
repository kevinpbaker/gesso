...
| [`Alert`](/components/alert) | function | `gesso-components` | A banner that stays on the screen, in one of the three tones. |
...
| [`Breadcrumb`](/components/breadcrumb) | function | `gesso-components` | The trail to where you are, whose last crumb is not a link. |
| [`BreadcrumbItem`](/components/breadcrumb) | interface | `gesso-components` | One crumb: its value and the word on it. |
...
| [`Link`](/components/link) | function | `gesso-components` | Words that go somewhere, announced as a link rather than a button. |
...
| [`Meter`](/components/meter) | function | `gesso-components` | A measurement inside a known range, with bands that decide its tone. |
...
| [`Pagination`](/components/pagination) | function | `gesso-components` | The strip of page numbers under a paged list. |
...
| [`SegmentedControl`](/components/segmented-control) | function | `gesso-components` | One value chosen from a few, laid out as one track of segments. |
| [`SegmentedOption`](/components/segmented-control) | interface | `gesso-components` | One segment: its value, its label and whether it can be chosen. |
