...
| `tick`              | Every `requestAnimationFrame`, but only while the runtime has asked for the loop. A worker with a frame callback of its own never asks | `time`, the rAF timestamp                                                                                                                                   |
...
| `frameLoop`     | Whenever the runtime starts or stops wanting the shell to forward refreshes. Never sent where the worker has its own frame callback | `running`; the shell runs or cancels its `requestAnimationFrame` loop                                                                            |
