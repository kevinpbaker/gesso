...
export function mountAnimationExampleRoute(host: HTMLElement): () => void {
  ...
  const app = createApp({
    ...
    renderWorker: () =>
      new Worker(new URL('../examples/AnimationExampleWorker.ts', import.meta.url), { type: 'module' }),
    appLogicWorker: () =>
      new Worker(new URL('../examples/board/BoardAppWorker.ts', import.meta.url), { type: 'module' }),
    onFrame: metrics => {
      frames++;
