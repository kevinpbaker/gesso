...
import type { PlaygroundDirection } from './PlaygroundState';

/**
 * Wires the LayoutPlayground harness to the DOM: controls, preview,
 * debug panel and the resize observer.
 *
...
export function mountPlayground(host: HTMLElement): void {
  ...
  const debugView = new LayoutDebugView(requireElement('.pg-canvas'), node => {
    selectedId = node.id;
    refresh();
  });
  ...
  function updateScrollStats(): void {
    const scroll = playground.inspect().find(entry => entry.node.type === UiNodeType.ScrollView);
  ...
  function updateSelection(): void {
    ...
    detail.textContent = `id ${info.node.id} \u00b7 ${info.node.type} \u00b7 box ${box} \u00b7 created: ${created}`;
  }
...
function renderTemplate(): string {
  return `
  <div class="pg-app">
    <header class="pg-header">Layout Playground</header>
    <main class="pg-main">
