...
| Text breaks where Chrome breaks         | **107** paragraphs in six Noto faces (Latin, CJK, Arabic, Hebrew, Devanagari, Thai), laid out by Chrome and by `layoutParagraph` over Chrome's own run widths: 101 agree on every line's offsets, ink and baseline, including hyphens, dashes, ligatures, kerning, kinsoku, Hangul, right-to-left alignment and Thai dictionary breaks; the 6 that differ are pinned by name | `pnpm test:run` · regenerate with `pnpm fixtures:text`   |
