...
| Text breaks where Chrome breaks         | **115** paragraphs in seven Noto faces (Latin, CJK, Arabic, Hebrew, Devanagari, Thai, colour emoji), laid out by Chrome and by `layoutParagraph` over Chrome's own run widths: 109 agree on every line's offsets, ink and baseline, including hyphens, dashes, ligatures, kerning, kinsoku, Hangul, right-to-left alignment, Thai dictionary breaks and emoji sequences; the 6 that differ are pinned by name. The WebGPU shaper's word order is checked against Chrome's recorded cluster positions for every right-to-left fixture | `pnpm test:run` · regenerate with `pnpm fixtures:text`   |
...
| Every control works from the keyboard   | One form holding every control in the library, tabbed through from nothing: fourteen stops, each a named control in order, each operated by its role's key with the result read from the semantics tree; and Enter or Space presses any focused button                                                                                                                                                                                                                                                                               | `Keyboard.spec.ts` in `@gesso/components`                |
| A screen reader has something to read   | Chrome's own **computed accessibility tree** for three example routes, written to a committed report each (`docs/accessibility/`): every node with its role, name, states and value, every control named, and the order the Tab key reaches them in, walked by pressing it; a synthesised press reaches the app, and a status is a live region                                                                                                                                                                                       | `pnpm check:a11y`                                        |
