...
| Text breaks where Chrome breaks         | **96** paragraphs in Noto Sans, Noto Sans CJK, Noto Sans Arabic and Noto Sans Hebrew, laid out by Chrome and by `layoutParagraph` over Chrome's own run widths: 91 agree on every line's offsets, ink and baseline, including hyphens, dashes, ligatures, kerning, kinsoku, Hangul and right-to-left alignment; the 5 that differ are pinned by name | `pnpm test:run` · regenerate with `pnpm fixtures:text`   |
