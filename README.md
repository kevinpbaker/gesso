...
| Text breaks where Chrome breaks         | **58** paragraphs in Noto Sans, laid out by Chrome and by `layoutParagraph` over Chrome's own run widths: 48 agree on every line's offsets, ink and baseline within 0.1 px; the 10 line-breaking rules Gesso lacks (hyphens, dashes, soft hyphens, tabs) are pinned by name | `pnpm test:run` · regenerate with `pnpm fixtures:text`   |
...
| `layout`      | `LayoutEngine`, `ParagraphLayout`, `GridLayout`, `LayoutExplanation`, typed lengths, `conformance/` and `textConformance/` fixtures               |
