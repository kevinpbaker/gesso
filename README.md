...
| Text breaks where Chrome breaks         | **68** paragraphs in Noto Sans, laid out by Chrome and by `layoutParagraph` over Chrome's own run widths: 63 agree on every line's offsets, ink and baseline within 0.1 px, including hyphens, dashes, ligatures and kerning; the 5 that differ (soft hyphens, tab stops, two Gesso choices, one Chrome quirk) are pinned by name | `pnpm test:run` · regenerate with `pnpm fixtures:text`   |
