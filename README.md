...
- **Overflow and scrolling.** `overflow: 'hidden' | 'scroll' | 'auto'` on any container, rounded clipping, `position: 'sticky'`, draggable overlay scrollbars that fade, and `scrollIntoView` wired to keyboard focus.
- **Flexbox, complete.** Cross-axis `stretch` by default, automatic minimum size, freeze-and-redistribute resolution, `flexWrap` + `alignContent`, reverse directions and RTL, `margin: auto`, `flex: n`, `aspectRatio`, and typed lengths (`percent(50)`, `auto`) — each verified against Chrome.
